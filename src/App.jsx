...
const HOW_TO_PLAY_KEY = 'pathways-how-to-play-seen';
...
export default function App() {
  ...
  useEffect(() => {
    if (gameStatus === 'won') {
      ...
      recordTodayShare('pathways', dateKey, generateShareText());
    }
  }, [gameStatus]);
  ...
  const Logo = () => (
    <h1 className={styles.logo}>
      <GameLogo />
      <span className={styles.logoText}>Pathways</span>
    </h1>
  );
  ...
  const totalPairs = puzzle.colors.length;
  const pairsConnected = completedColors.size;
      ...
      <main className={styles.main}>
        <div className={styles.statusBar}>
          ...
          <div className={styles.pairsBlock}>
            ...
            <span className={styles.pairsValue}>{pairsConnected}/{totalPairs}</span>
          </div>
        </div>
        ...
        <PathwayGrid
          ...
          onInterference={recordInterference}
        />

        {gameStatus === 'playing' && (
          <p className={styles.hint}>
            Drag between matching dots to connect them — fill every cell, don't cross your own path
          </p>
        )}
      </main>
