...
export default function WinScreen({ puzzle, puzzleNumber, elapsedSeconds, interference, generateShareText, stats, winPct, onDismiss }) {
  ...
  const rating = getRating(interference, elapsedSeconds);
      ...
      <div className={styles.modal}>
        <div className={styles.sparkContainer} aria-hidden="true">
          {puzzle.colors.map((color, i) => (
            <span key={color.id} className={styles.spark} style={{
              left: `${10 + i * (80 / Math.max(puzzle.colors.length - 1, 1))}%`,
              top: `${Math.random() * 100}%`,
              background: color.hex,
              boxShadow: `0 0 4px ${color.hex}, 0 0 8px ${color.hex}88`,
        ...
        <div className={styles.metricsRow}>
          ...
          <div className={styles.metric}>
            <span className={styles.metricValue}>{interference}</span>
            <span className={styles.metricLabel}>{interference === 1 ? 'Redraw' : 'Redraws'}</span>
          </div>
          <div className={styles.metricDivider} />
          <div className={styles.metric}>
            <span className={styles.metricValue}>{puzzle.colors.length}</span>
